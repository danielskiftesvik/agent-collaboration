...
If no job id or `--latest` was passed, render the output as a compact Markdown table. Otherwise present the selected job's full detail. Plain reads do not acquire the state lock; use `--refresh` to update liveness or `--wait` to block. Do not add commentary.
