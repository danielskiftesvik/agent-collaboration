...
| `AGENT_COLLAB_QUIET_AFTER` | Soft quiet warn in **seconds** (default 45). Live workers with no progress for this long get `health.quiet` / `state:"quiet"` on `status` — does **not** kill (idle kill still uses `AGENT_COLLAB_IDLE_TIMEOUT`) |
