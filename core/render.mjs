...
export function renderJob(job) {
  ...
  return [
    ...
    running && health ? `health   ${health.healthy ? (health.quiet ? "quiet (live, no recent progress)" : "live and within budget (not stalled)") : health.state}` : null,
    running ? `progress ${health?.lastProgressAt || job.lastProgressAt || job.heartbeatAt || "unknown"}${health?.lastProgressKind || job.lastProgressKind ? ` (${health?.lastProgressKind || job.lastProgressKind})` : ""}${health?.secondsSinceProgress != null ? ` — ${health.secondsSinceProgress}s ago` : ""}` : null,
