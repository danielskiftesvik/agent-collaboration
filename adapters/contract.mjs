...
//   buildRetryCommand({ role, repairBrief, workspace, timeoutMs, sessionId? }) -> { command, args, env? } | null
//       A repair / null-turn continue that CONTINUES/RESUMES the worker's existing
//       thread with a short ask. Return null to fall back to a fresh buildCommand
//       re-send. (codex: `task --resume-last`; opencode: `run --session <id>` —
//       never bare `--continue`.) `sessionId` is prior-attempt telemetry when known.
